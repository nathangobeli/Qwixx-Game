...
export const Row = ({ playerIndex, color, rowState }: RowProps) => {
    ...
    return (
        <div className={`flex w-full items-center rounded-r-2xl rounded-l-md border sm:border-2 shadow-sm my-px sm:my-1 flex-1 min-h-0 ${colorMap[color]}`}>
            <div className={`w-4 h-4 sm:w-6 sm:h-6 rounded-full ml-1 mr-1 flex items-center justify-center border border-white/50 bg-white/20 text-[8px] sm:text-[10px]`}>
                {/* Placeholder for lock icon */}
                {isColorLocked ? '🔒' : ''}
            </div>

            <div className="flex-1 flex justify-between pr-0.5 sm:pr-1 h-full py-0.5 min-h-0">
                {numbers.map((num) => {
                    ...
                    return (
                        <div
                            ...
                            className={`flex-1 mx-px rounded flex items-center justify-center font-bold text-xs sm:text-base shadow-inner border border-black/10 transition-colors
              ...
              `}
                ...
                {/* The Lock Box */}
                <div className={`w-5 sm:w-8 ml-0.5 rounded-full flex items-center justify-center font-bold text-xs sm:text-base shadow-inner border border-black/10
