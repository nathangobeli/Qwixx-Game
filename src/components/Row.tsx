...
export const Row = ({ playerIndex, color, rowState }: RowProps) => {
    ...
    return (
        <div className={`flex w-full items-stretch rounded-r-2xl rounded-l-md border sm:border-2 shadow-sm my-px md:my-1 flex-1 min-h-0 ${colorMap[color]}`}>
            <div className={`w-3 h-3 md:w-5 md:h-5 rounded-full ml-1 mr-1 self-center flex items-center justify-center border border-white/50 bg-white/20 text-[6px] md:text-[10px]`}>
                {/* Placeholder for lock icon */}
                {isColorLocked ? '🔒' : ''}
            </div>

            <div className="flex-1 flex justify-between pr-0.5 h-full py-0.5 md:py-1 min-h-0 items-stretch gap-0.5">
                {numbers.map((num) => {
                    ...
                    return (
                        <div
                            ...
                            className={`flex-1 rounded flex items-center justify-center font-bold shadow-inner border border-black/10 transition-colors
                ${isMarked ? 'bg-black/80 text-white' : (canMark ? 'bg-white/90 hover:bg-white cursor-pointer active:scale-95' : 'bg-white/50 opacity-50')}
              `}
                            style={{ fontSize: 'clamp(0.8rem, 4cqi, 2.5rem)', minWidth: 0 }}
                        >
                            {isMarked ? 'X' : num}
                        </div>
                    );
                ...
                {/* The Lock Box */}
                <div className={`ml-0.5 rounded-full flex items-center justify-center font-bold shadow-inner border border-black/10
            ${isColorLocked ? 'bg-black/80 text-white' : 'bg-white/90'}
        `} style={{ width: 'clamp(1.5rem, 6cqi, 4rem)', fontSize: 'clamp(0.8rem, 4cqi, 2.5rem)' }}>
                    {isColorLocked ? 'X' : '🔒'}
                </div>        </div>        </div>
    );
};
